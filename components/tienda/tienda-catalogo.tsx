...
export default function TiendaCatalogo() {
  ...
  const [newProduct, setNewProduct] = useState({
    ...
    precio_pesos: 0,
    stock: 0,
    vendedor: "Edificio" as "Edificio" | "Residente" | "Externo"
  })
  ...
  const agregarNuevoProducto = () => {
    if (!newProduct.nombre || !newProduct.categoria || newProduct.precio_coins <= 0) {
      ...
      <Card className="bg-card border-border shadow-md">
        <CardHeader className="border-b border-border/30 pb-3">
          <div className="flex justify-between items-start">
            ...
            <Button onClick={() => setShowNewProductDialog(true)} className="bg-primary hover:bg-primary/90">
      ...
      <Dialog open={showNewProductDialog} onOpenChange={setShowNewProductDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Agrega un nuevo producto al inventario del edificio
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="precio_coins">Precio en HabiCoins *</Label>
                <Input
                  id="precio_coins"
                  type="number"
                  value={newProduct.precio_coins || ""}
                  onChange={(e) => setNewProduct(prev => ({ ...prev, precio_coins: parseInt(e.target.value) || 0 }))}
                  placeholder="150"
                  min="1"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="precio_pesos"
                  type="number"
                  value={newProduct.precio_pesos || ""}
                  onChange={(e) => setNewProduct(prev => ({ ...prev, precio_pesos: parseInt(e.target.value) || 0 }))}
                  placeholder="450"
                  min="0"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select 
                  value={newProduct.vendedor} 
                  onValueChange={(value) => setNewProduct(prev => ({ ...prev, vendedor: value as "Edificio" | "Residente" | "Externo" }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="bg-muted/50 p-4 rounded-lg">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium">{newProduct.nombre || "Nombre del producto"}</span>
                {newProduct.categoria && (
                  <Badge className={`${getCategoriaColor(newProduct.categoria)} text-white text-xs`}>
                    {newProduct.categoria}
                  </Badge>
                )}
              </div>
              ...
              <div className="flex gap-4 text-sm">
                ...
                <span><strong>Stock:</strong> {newProduct.stock} unidades</span>
              </div>
            </div>
          </div>
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={!newProduct.nombre || !newProduct.categoria || newProduct.precio_coins <= 0}
